...
const CompetedList = ({completedData}) => {
  const { id, task, date, taskStatus, taskCategory } = completedData;
  ...
  return (
<>
      <ul draggable onDragStart={(e) =>handleDragStart(e, completedData, id)} onDragEnd={(e) => handleDragEnd(e)} className="flex  py-2 font-semibold text-gray-600 px-4 sm:cursor-grab sm:active:opacity-70 sm:active:border sm:active:border-gray-900 sm:active:shadow-[2px_1px_10px_black]">
        ...
        <li className="w-64 hidden sm:block">
          <span className="px-2 py-1 bg-green-500 text-white/90">{taskStatus}</span>
        </li>
      ...
      <EditTask toggle={toggle} setToggle={setToggle} singleEditData = {completedData} />
    </>
  )
}
